...
from .tools import _unit_edges_from_block_edges, graph_to_batch_nx
...
class EPT(nn.Module):
    def __init__(self, hidden_size, ffn_size, n_rbf, cutoff=7.0, z_requires_grad=False, 
                 ...
                 efficient=False, vector_act="none") -> None:
        super().__init__()
        ...
        self.encoder = Transformer(
            ...
            layer_norm = 'pre' if pre_norm else 'post', sparse_k = sparse_k, efficient = efficient, vector_act = vector_act
        )

    def forward(self, H, Z, batch_id, edges, edge_attr=None, block_id=None):
        if block_id is None:
            block_id = torch.arange(batch_id.shape[0], device=batch_id.device)
...
class Transformer(nn.Module):
    ...
    def __init__(self, d_hidden, d_ffn, n_heads, n_layers, n_rbf, d_edge, cutoff=7.0, act_fn=nn.SiLU(), layer_norm = 'pre', residual = True, use_edge_feat = False, local_mask = False, attn_bias = True,
                 sparse_k=3, efficient=False, vector_act='none'):
        super().__init__()
    ...
    def forward(self, H, Z, block_id, batch_id, edges, edge_attr=None):
        ...
        if edge_attr is None:
            edge_feat = torch.cat([H[unit_row], H[unit_col], dis_feat], dim=-1)
        else:
...
if __name__ == '__main__':
    ...
    model = Transformer(d_hidden, d_ffn, n_heads, n_layers, n_rbf, d_edge=d_edge, use_ieconv=True, use_edge_feat=True)
    model.to(device)
    model.eval()
