...
class AutoEncoder(nn.Module):
    def __init__(
        ...
        self.non_mask_loss_ratio = non_mask_loss_ratio 
        ...
        # TODO: edge type (inter and intra), or segment embedding
    ...
    def rsample(self, H, Z, Z_centers, no_randomness=False):
        ...
        if self.mode != 'fixbb':
            ...
            Z_kl_loss = -0.5 * torch.sum(1.0 + Z_log_var - Z_mean_delta * Z_mean_delta - torch.exp(Z_log_var)) / data_size
    ...
    @torch.no_grad()
    def _replace_pep(self, S, atom_mask, mask_generate):
        if not hasattr(self, 'replace_ratio') or self.replace_ratio <= 0:
            return S, atom_mask
        assert self.replace_ratio > 0
        S, atom_mask = S.clone(), atom_mask.clone()
        pep_S = S[mask_generate]
        do_replace = torch.logical_and(
            torch.rand_like(pep_S, dtype=torch.float) < self.replace_ratio,
            torch.logical_not(pep_S == self.mask_id)
        )
        pep_S[do_replace] = self.s_remap[torch.randint_like(pep_S[do_replace], high=len(self.s_remap) - 1) + 1] # 0 is UNK

        S[mask_generate] = pep_S
        atom_mask[mask_generate] = self._remove_sidechain_atom_mask(atom_mask[mask_generate], do_replace)
    ...
    def decode(self, X, S, H, Z, mask, position_ids, lengths, atom_mask, teacher_forcing):
        ...
        true_S = S[mask].clone()
        # n_channel = X.shape[1]
        # eliminate the possibility to get access to the ground truth
        # if self.mode == 'fixbb':
        #     # X[mask] = X[mask][:, self.ca_channel_idx].unsqueeze(1).repeat(1, n_channel, 1)
    ...
    def forward(self, X, S, mask, position_ids, lengths, atom_mask, teacher_forcing=True):
        ...
        input_S, input_atom_mask = self._replace_pep(input_S, input_atom_mask, mask)

        H, Z, H_kl_loss, Z_kl_loss = self.encode(X, input_S, mask, position_ids, lengths, input_atom_mask)
        
        # ca_mask = atom_mask[mask][:, self.ca_channel_idx]
        # coord_reg_loss = F.mse_loss(Z.squeeze(1)[ca_mask], true_X[:, self.ca_channel_idx][ca_mask])
        if self.mode != 'fixbb':
            coord_reg_loss = F.mse_loss(Z, self._get_latent_channel_anchors(true_X, atom_mask[mask]))
        else:
            coord_reg_loss = 0
        # coord_reg_loss = F.mse_loss(Z, self._get_latent_channels(true_X, atom_mask[mask]))
        ...
        if self.mode != 'fixseq':
            if getattr(self, 'non_mask_loss_ratio', 1.0):
                cal_loss_mask = torch.logical_or(input_S[mask] == self.mask_id, torch.rand_like(true_S, dtype=torch.float) < self.non_mask_loss_ratio)
                seq_recon_loss = F.cross_entropy(recon_S_logits[cal_loss_mask], self.s_map[true_S[cal_loss_mask]])
            else:
    ...
    @torch.no_grad()
    def test(self, X, S, mask, position_ids, lengths, atom_mask, given_laten_H=None, given_latent_X=None, return_tensor=False, allow_unk=False, optimize_sidechain=True, idealize=False):

        if given_laten_H is None and given_latent_X is None:
            # encode: H (N*d), Z (N*3)
            H, Z, _, _ = self.encode(X, S, mask, position_ids, lengths, atom_mask)
            # print(torch.sqrt(torch.mean(((Z - X[:, self.ca_channel_idx][mask]) ** 2).sum(-1))))
        ...
        #     fname_exist[fname] = True
        # for i in range(100):
        #     fname = f'{i}.pkl'
        #     if fname not in fname_exist:
        #         break
        # fname = os.path.join(save_dir, fname)
        # with open(fname, 'wb') as fout:
        #     pkl.dump((ca_x.tolist(), lengths.tolist(), mask.tolist()), fout)
