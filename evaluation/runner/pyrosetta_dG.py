...
def run_pyrosetta_relax(task: Task):
    ...
    try:
        pyrosetta_fastrelax(task.in_path, out_path, task.pep_chain)
...
@ray.remote(num_cpus=1)
def run_pyrosetta_dg(task: Task):
    if not task.can_proceed():
        return task
    try:
        dG = pyrosetta_interface_energy(task.current_path, task.rec_chains, [task.pep_chain])
        task.set_data(dG)
        task.mark_success()
    except Exception as e:
...
def main(args):
    ...
    out_file = os.path.join(args.root_dir, args.result_dir, 'pyrosetta_dG.txt')
    if os.path.exists(out_file):
        os.remove(out_file)
